...
export async function fetchProfile(username) {
  const data = await jsonRequest(`/api/profiles/${username}`);
  return data?.profile ?? null;
}

export async function fetchMyProfile() {
  const data = await jsonRequest("/api/profiles/me");
  return data?.profile ?? null;
}
...
export async function fetchGroups() {
  const data = await jsonRequest("/api/groups");
  return data?.groups ?? [];
}

export async function fetchMyGroups() {
  const data = await jsonRequest("/api/groups/me");
  return data?.groups ?? [];
}
...
export async function joinGroup(groupId) {
  await jsonRequest(`/api/groups/${groupId}/join`, {
    method: "POST",
  });
}

export async function fetchGroupMessages(groupId) {
  const data = await jsonRequest(`/api/groups/${groupId}/messages`);
...
export async function followUser(userId) {
  await jsonRequest(`/api/users/${userId}/follow`, {
    method: "POST",
  });
}

export async function fetchMyStats() {
  const data = await jsonRequest("/api/profiles/me/stats");
  return data ?? { following: 0, followers: 0 };
}

export async function fetchUserStats(userId) {
  const data = await jsonRequest(`/api/profiles/${userId}/stats`);
  return data ?? { following: 0, followers: 0, isFollowing: false };
}

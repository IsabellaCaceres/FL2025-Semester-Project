...
import coverAssetsModule from './cover-assets.js'

const coverAssetNamespace =
  (coverAssetsModule && typeof coverAssetsModule === 'object'
    ? coverAssetsModule
...
function resolveCoverUri(uri) {
  if (!uri) return null
  // If it's already a data URI or absolute URL, use it as-is
  if (uri.startsWith('data:') || uri.startsWith('http://') || uri.startsWith('https://') || uri.startsWith('file://')) {
    return uri
  }
  // For relative paths, use as-is (works on web)
  // For native platforms, you might need to prepend with asset path or use require()
  // For now, assuming relative paths work (web) or assets are bundled
  return uri
}
...
function normalizeBook(entry) {
  ...
  const coverEntry = coverAssets?.[entry.id] ?? null
  const coverUri =
    coverEntry?.uri ??
    entry.cover?.uri ??
    null
  const coverSource =
    coverEntry?.source ??
    (coverUri ? { uri: resolveCoverUri(coverUri) } : null)
  ...
  const entryGenres = Array.isArray(entry.genres)
    ? entry.genres.map((genre) => cleanLabel(genre)).filter(Boolean)
    : []
  const genres = uniqueStrings([...normalizeGenres(subjects), ...entryGenres])
  ...
  const book = {
    ...
    coverAsset: coverEntry?.source ?? null,
    coverSource,
    metadata: {
      ...metadata,
      keywords,

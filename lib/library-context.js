...
export function LibraryProvider({ user, children }) {
  ...
  const recommendationModeRef = useRef('fallback');
  ...
  useEffect(() => {
    if (recommendationModeRef.current !== 'remote') {
  ...
  const refreshRecommendations = useCallback(
    async (options = {}) => {
      ...
      if (!userId) {
        recommendationModeRef.current = 'fallback';
        setRecommended([]);
        return;
      }
      try {
        const remote = await fetchSemanticRecommendations(12);
        if (signal?.aborted) return;
        const mapped = [];
        if (Array.isArray(remote)) {
          for (const item of remote) {
            const contentHash = item?.bookId ?? item?.book_id ?? null;
            if (!contentHash) continue;
            const book = catalogIndex.get(contentHash);
            if (book) mapped.push(book);
          }
        }
        if (mapped.length > 0) {
          recommendationModeRef.current = 'remote';
        ...
        if (signal?.aborted) return;
        console.warn('[library-context] Failed to load AI recommendations', error);
        recommendationModeRef.current = 'fallback';
        setRecommended(fallbackRecommended);
      }
    },
    [userId, fallbackRecommended]
  );
  ...
  useEffect(() => {
    ...
    recommendationModeRef.current = 'fallback';
    setRecommended(fallbackRecommended);
    refreshRecommendations({ signal: controller?.signal });
    return () => controller?.abort?.();
  }, [userId, libraryIds, fallbackRecommended, refreshRecommendations]);
